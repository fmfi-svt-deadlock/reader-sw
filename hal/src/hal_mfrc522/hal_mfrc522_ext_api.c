...
bool mfrc522PerformSelftest(Mfrc522Driver *mdp) {
    ...
    uint8_t i;
    for (i = 0; i < 25; i++) {
        mfrc522_write_register(mdp, FIFODataReg, 0x00);
    }
    mfrc522_command(mdp, Command_Mem);

    // Enable the self test
    mfrc522_write_register(mdp, AutoTestReg,
    ...
    // Read the result
    uint8_t result[64];
    mfrc522_read_register_burst(mdp, FIFODataReg, result, 64);

    // Restore the config lost during reset
    mfrc522Reconfig(mdp, mdp->current_config);

    switch(mfrc522_read_register(mdp, VersionReg)) {
        case 0x90:
